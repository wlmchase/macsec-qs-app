...
# Server setup
def start_server():
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM) # init TCP connection
    server_socket.bind((HOST, PORT))  # Bind to host, and port
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    server_socket.listen(CONNECTION_LIMIT) # set connection limit
    print(f"Server listening on port {PORT} ")
    
    # Initialize PQ KEM and Signature
    ...
    # Package server public keys in dictionary
    data = {
    ...
    message = json.loads(data) # Extract data from JSON string
    client_public_kem = base64.b64decode(message['kem_pk']) # Get client KEM public key
    client_public_sign = base64.b64decode(message['sign_pk']) # Get client Signature public key

    ciphertext, shared_secret = kem_algorithm.encap_secret(client_public_kem)  # Compute shared secret
    
    # Send client ciphertext
    client_socket.sendall(ciphertext)

    # Use the shared secret as the AES key
    global AES_KEY
    AES_KEY = shared_secret  # Use first 256 bits for AES key 

    ##### SENDING & RECEIVING THREADS #####
    sending_message_thread = threading.Thread(target=send_message, args=(client_socket, sign_algorithm, ))
    receive_message_thread = threading.Thread(target=receive_message, args=(client_socket, sign_algorithm, client_public_sign, ))
